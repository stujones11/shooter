local function throw_hook(itemstack, user, vel)
	...
	if pos and dir and yaw then
		...
		pos.y = pos.y + 1.5
		local obj = minetest.add_entity(pos, "hook:hook")
		if obj then
...
minetest.register_entity("hook:hook", {
	physical = true,
	timer = 0,
	visual = "wielditem",
	visual_size = {x=1/2, y=1/2},
	textures = {"hook:grapple_hook"},
	player = nil,
	itemstack = "",
...
minetest.register_tool("hook:grapple_hook", {
...
minetest.register_tool("hook:grapple_gun", {
	description = "Grappling Gun",
	inventory_image = "hook_hook_gun.png",
	on_use = function(itemstack, user, pointed_thing)
		local inv = user:get_inventory()
		if inv:contains_item("main", "hook:grapple_hook") and 
				inv:contains_item("main", "tnt:gunpowder") then
			inv:remove_item("main", "tnt:gunpowder")
			minetest.sound_play("shooter_reload", {object=user})
			local stack = inv:remove_item("main", "hook:grapple_hook")
			itemstack = "hook:grapple_gun_loaded 1 "..stack:get_wear()
		else
...
minetest.register_tool("hook:grapple_gun_loaded", {
	description = "Grappling Gun",
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		itemstack = ItemStack("hook:grapple_hook 1 "..itemstack:get_wear())
		throw_hook(itemstack, user, 20)
		return "hook:grapple_gun"
	end,
})

if SHOOTER_ENABLE_CRAFTING == true then
	minetest.register_craft({
		output = "hook:grapple_hook",
		recipe = {
...
minetest.register_alias("shooter:grapple_hook", "hook:grapple_hook")
minetest.register_alias("shooter:grapple_gun", "hook:grapple_gun")
minetest.register_alias("shooter:grapple_gun_loaded", "hook:grapple_gun_loaded")
